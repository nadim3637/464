...
export const executeWithRotation = async <T>(
    ...
    try {
        ...
        if (usage) {
            const settingsStr = await storage.getItem('nst_system_settings');
...
export const fetchChapters = async (
  ...
  try {
      const s = await storage.getItem('nst_system_settings');
      if (s) { const p = typeof s === 'string' ? JSON.parse(s) : s; if(p.aiModel) modelName = p.aiModel; }
  } catch(e){}
...
export const fetchLessonContent = async (
  ...
  try {
      const stored = await storage.getItem('nst_system_settings');
      if (stored) {
          const s = (typeof stored === 'string' ? JSON.parse(stored) : stored) as SystemSettings;
