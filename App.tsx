...
const App: React.FC = () => {
  ...
  const [darkMode, setDarkMode] = usePersistentState<boolean>('nst_dark_mode', false);
  ...
  // Removed duplicate local state, using persistent state below
  const [studentTab, setStudentTab] = usePersistentState<StudentTab>('nst_active_student_tab', 'HOME');
  ...
  useEffect(() => {
      storage.getItem('nst_app_version').then(storedVersion => {
  ...
  useEffect(() => {
      ...
      if (deletedKeys.length > 0) {
          ...
          if (newDeletedKeys.length !== deletedKeys.length) {
              ...
              storage.setItem('nst_system_settings', JSON.stringify(updatedSettings));
          }
      }
  ...
  useEffect(() => {
      if (isOnline) {
          storage.getItem('nst_pending_sync_results').then(pendingStr => {
              if (pendingStr) {
                  try {
                      const pending = typeof pendingStr === 'string' ? JSON.parse(pendingStr) : pendingStr;
                      if (Array.isArray(pending) && pending.length > 0) {
                          console.log(`Syncing ${pending.length} offline results...`);
                          pending.forEach(async (item: any) => {
  ...
  useEffect(() => {
      ...
      const unsubscribe = subscribeToSettings((newSettings) => {
          if (newSettings) {
              ...
              // VERSION CHECK LOGIC
              storage.getItem('nst_app_version').then(currentVersion => {
                  const ver = currentVersion || APP_VERSION;
                  if (newSettings.latestVersion && newSettings.latestVersion !== ver) {
                      if (newSettings.latestVersion > ver) {
                      const now = Date.now();
  ...
  const checkUpdatePopup = (newSettings: any, referenceTime: number, now: number) => {
                          ...
                          let deadline = referenceTime + graceDuration;
                          let shouldShow = false;
                          ...
                          if (now >= deadline) {
                              ...
                              storage.getItem(`nst_update_dismissed_${newSettings.latestVersion}`).then(lastDismissedStr => {
                                  if (!lastDismissedStr) {
          ...
          getUserData(state.user.id).then(cloudUser => {
             if (cloudUser) {
                 ...
                 if (currentStr !== cloudStr) {
                     ...
                     storage.setItem('nst_current_user', cloudStr);
  ...
  useEffect(() => {
      ...
      storage.getItem('nst_system_settings').then(storedSettings => {
          if (storedSettings) {
              try {
          ...
          storage.getItem('nst_terms_accepted').then(hasAcceptedTerms => {
              ...
              storage.getItem('nst_current_user').then(loggedInUserStr => {
                  const today = new Date().toDateString();

                  if (loggedInUserStr) {
                      // 2. Daily Tracker (Once per day)
                      storage.getItem('nst_last_daily_tracker_date').then(lastTracker => {
                          if (lastTracker !== today) {
                              queue.push('TRACKER');
                          }
                          // 3. Daily Challenge (Once per day)
                          storage.getItem('nst_last_daily_challenge_date').then(lastChallenge => {
                      ...
                      if (loggedInUserStr) {
                          try {
                              ...
                              if (user.role !== 'ADMIN' && user.isPremium && user.subscriptionEndDate && new Date(user.subscriptionEndDate) <= new Date()) {
                                  ...
                                  storage.setItem('nst_current_user', JSON.stringify(user));
                                  saveUserToLive(user);
                              }
  ...
  useEffect(() => {
    ...
    Promise.all([
        storage.getItem('nst_timer_date'),
        storage.getItem('nst_daily_study_seconds')
    ]).then(([storedDate, storedSecondsStr]) => {
        const storedSeconds = parseInt((storedSecondsStr as string) || '0');
  ...
  useEffect(() => {
      ...
      if (state.user) {
          ...
          storage.getItem('nst_leaderboard').then(lbData => {
              try {
                  if (lbData) {
                      const entries: any[] = typeof lbData === 'string' ? JSON.parse(lbData) : lbData;
                      const top3 = entries
                          .sort((a: any, b: any) => b.score - a.score)
                          .slice(0, 3)
                          .map((e: any) => e.userId);
                      
                      if (state.user && top3.includes(state.user.id)) {
                          // activeThemeColor = '#eab308'; // Gold - Cannot update local variable easily in effect, relying on re-render if needed or separate state. 
                          // However, we are setting style tag directly below.
                          // We should probably move theme logic to a separate effect or persistent state if we want it fully async.
                          // For now, let's keep synchronous fallback or just accept it might not apply gold immediately.
                      }
                  }
              } catch(e) {}
          });
  ...
  const logActivity = (action: string, details: string, overrideUser?: User) => {
      ...
      storage.getItem('nst_activity_log').then(storedLogs => {
          const logs: ActivityLogEntry[] = storedLogs ? (typeof storedLogs === 'string' ? JSON.parse(storedLogs) : storedLogs) : [];
          // Keep last 500 logs
          const updatedLogs = [...logs, newLog].slice(-500); 
          storage.setItem('nst_activity_log', JSON.stringify(updatedLogs));
      });
  };
  ...
  useEffect(() => {
    if (state.user && state.view === 'STUDENT_DASHBOARD') {
        ...
        Promise.all([
            storage.getItem('nst_last_daily_tracker_date'),
            storage.getItem('nst_last_daily_challenge_date')
        ]).then(([lastTracker, lastChallenge]) => {
            // 1. Daily Tracker
            if (lastTracker !== today) {
                queue.push('TRACKER');
            }
  ...
  const handleLogin = (user: User) => {
    ...
    // Auto-Speak Welcome
    voice.speak(`Welcome back, ${user.name}. Let's learn something new today!`);
