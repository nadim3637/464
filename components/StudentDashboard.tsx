...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter, isDarkMode, onToggleDarkMode }) => {
  ...
  // WRAPPED TAB CHANGE FOR AUDIO GUIDE
  const handleTabChange = (tab: StudentTab) => {
      // Check Voice Guide Preference
      storage.getItem('nst_voice_enabled').then(enabled => {
          if (enabled !== 'false') {
              const script = settings?.audioGuideScripts?.[tab] || `Opening ${tab.toLowerCase().replace('_', ' ')} section.`;
              voice.speak(script);
          }
      });
      onTabChange(tab);
  ...
  useEffect(() => {
      ...
      const unsub = onValue(q, snap => {
          ...
          if (data) {
              ...
              storage.getItem('nst_last_read_update').then(lastReadStr => {
                  const lastRead = lastReadStr || '0';
                  if (new Date(latest.timestamp).getTime() > Number(lastRead)) {
                      ...
                      storage.getItem(alertKey).then(shown => {
                          if (!shown) {
  ...
  const [testAttempts, setTestAttempts] = usePersistentState<Record<string, any>>(`nst_test_attempts_${user.id}`, {});
  
  const [globalMessage, setGlobalMessage] = useState<string | null>(null);
  useEffect(() => {
      storage.getItem('nst_global_message').then(msg => setGlobalMessage(msg as string));
  }, []);
  ...
  useEffect(() => {
      ...
      if (isNew && !user.redeemedReferralCode) {
          storage.getItem(`referral_shown_${user.id}`).then(shown => {
              if (!shown) {
          ...
          if (now.getHours() >= 10) {
              ...
              if (!isGen) {
                  ...
                  try {
                      ...
                      const logsStr = await storage.getItem('nst_universal_analysis_logs');
                      const logs = logsStr ? (typeof logsStr === 'string' ? JSON.parse(logsStr) : logsStr) : [];
          ...
          if (!lastDaily) {
              ...
              if (user && user.classLevel && settings) {
                  try {
                      ...
                      await storage.setItem(`daily_challenge_data`, JSON.stringify(challenge));
                      await storage.setItem(`daily_challenge_gen_${dateStr}`, 'true');
              ...
              if (!lastWeekly) {
                  if (user && user.classLevel && settings) {
                      try {
                          ...
                          await storage.setItem(`weekly_challenge_data`, JSON.stringify(challenge));
                          await storage.setItem(`weekly_challenge_gen_${dateStr}`, 'true');
  ...
  const handleAiNotesGeneration = async () => {
      ...
      const storedUsage = await storage.getItem(usageKey);
      const currentUsage = parseInt((storedUsage as string) || '0');
  ...
  const handleContinueLearning = () => {
      // Find last accessed content from history
      const lastItem = user.usageHistory?.find(h => h.type === 'VIDEO' || h.type === 'MCQ' || h.type === 'PDF' || h.type === 'AUDIO');
      if (lastItem) {
          showAlert(`Resuming: ${lastItem.itemTitle}`, 'SUCCESS');
          // Since we don't have full chapter object here easily without fetching, we navigate to the relevant tab.
          // Ideally we should store last chapter details fully.
          // For now, we switch to the tab and try to select subject if possible, or just go to courses.
          // But wait, user.usageHistory items have itemId.
          if (onNavigateToChapter && lastItem.chapterId) {
               // We need subject name and chapter title. usageHistory has itemTitle (Chapter) and subject.
               onNavigateToChapter(lastItem.itemId || lastItem.chapterId, lastItem.itemTitle, lastItem.subject || 'General');
          } else {
               onTabChange('COURSES');
          }
      } else {
  ...
  useEffect(() => {
      ...
      storage.getItem(`nst_goal_${user.id}`).then(storedGoal => {
          if (storedGoal) {
              const hours = parseInt(storedGoal as string);
  ...
  useEffect(() => {
      ...
      Promise.all([
          storage.getItem(`activity_${user.id}_${yDateStr}`),
          storage.getItem(`reward_claimed_${user.id}_${yDateStr}`)
      ]).then(([yActivityStr, yClaimed]) => {
          const yActivity = parseInt((yActivityStr as string) || '0');
  ...
  useEffect(() => {
      const interval = setInterval(() => {
          ...
          storage.getItem(`first_day_ultra_${user.id}`).then(firstDayBonusClaimed => {
              ...
              if (accountAgeHours < 24 && dailyStudySeconds >= 3600 && !firstDayBonusClaimed) {
                  ...
                  storage.getItem('nst_users').then(storedUsersStr => {
                      const storedUsers = storedUsersStr ? (typeof storedUsersStr === 'string' ? JSON.parse(storedUsersStr) : storedUsersStr) : [];
  ...
  const handleUserUpdate = async (updatedUser: User) => {
      ...
      if (userIdx !== -1) {
          ...
          await storage.setItem('nst_users', JSON.stringify(storedUsers));
          
          if (!isImpersonating) {
              await storage.setItem('nst_current_user', JSON.stringify(updatedUser));
              ...
              <div className="space-y-6 pb-24">
                  <StudentHero 
                      ...
                      onStartTimer={(duration) => {
                          setDailyTargetSeconds(duration);
                          showAlert(`Target set to ${duration/60} mins`, 'SUCCESS');
                      }}
                  ...
                  <StudentExplore 
                      ...
                      onStartTour={() => setShowUserGuide(true)}
                  ...
                  <StudentMenu 
                      user={user} 
                      onNavigate={(tab) => {
                          if (tab === 'MARKSHEET') setShowMonthlyReport(true);
                          else if (tab === 'INBOX') setShowInbox(true);
                          else handleTabChange(tab);
                      }}
                      onLogout={() => {
                          storage.removeItem(`nst_user_${user.id}`).then(() => window.location.reload());
                      }}
                      unreadCount={unreadCount}
                      isAdmin={user.role === 'ADMIN' || !!isImpersonating}
                      onAdminSwitch={handleSwitchToAdmin}
                  />
              </div>
          );
      }
                    ...
                    <div className="flex gap-2">
                        ...
                        <button 
                            onClick={async () => {
                                ...
                                const existingStr = await storage.getItem('nst_demand_requests');
                                const existing = existingStr ? (typeof existingStr === 'string' ? JSON.parse(existingStr) : existingStr) : [];
                                existing.push(request);
                                await storage.setItem('nst_demand_requests', JSON.stringify(existing));
