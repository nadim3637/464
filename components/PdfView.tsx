...
import { FileText, Lock, ArrowLeft, Crown, Star, CheckCircle, AlertCircle, Globe, Maximize, Layers, HelpCircle, Minus, Plus, Volume2, Square, Zap, Sparkles } from 'lucide-react';
...
import { translateToHindi } from '../services/groq';
...
export const PdfView: React.FC<Props> = ({ 
  ...
  const [activeContentType, setActiveContentType] = useState<'FREE' | 'PREMIUM' | 'ULTRA' | null>(null);
  ...
  const handleSmartLanguageToggle = async (targetLang: 'ENGLISH' | 'HINDI') => {
      if (targetLang === activeLang) return;
      if (!activePdf) return;

      // 1. LEGACY: Dual content split
      if (activePdf.includes('|||')) {
          setActiveLang(targetLang);
          return;
      }

      // 2. SWITCH TO ENGLISH (Always available as base)
      if (targetLang === 'ENGLISH') {
          // If we are currently viewing Hindi (separate field), we need to revert to base content
          let baseContent = "";
          if (activeContentType === 'PREMIUM') {
              const htmlKey = syllabusMode === 'SCHOOL' ? 'schoolPremiumNotesHtml' : 'competitionPremiumNotesHtml';
              baseContent = contentData?.[htmlKey] || contentData?.premiumNotesHtml;
          } else {
              const htmlKey = syllabusMode === 'SCHOOL' ? 'schoolFreeNotesHtml' : 'competitionFreeNotesHtml';
              baseContent = contentData?.[htmlKey] || contentData?.freeNotesHtml;
          }
          
          if (baseContent) {
              setActivePdf(baseContent);
              setActiveLang('ENGLISH');
          }
          return;
      }

      // 3. SWITCH TO HINDI (Smart Check)
      if (targetLang === 'HINDI') {
          // Check if Hindi version exists in contentData
          let existingHindi = "";
          let targetKey = "";
          
          if (activeContentType === 'PREMIUM') {
              const htmlKeyHI = syllabusMode === 'SCHOOL' ? 'schoolPremiumNotesHtml_HI' : 'competitionPremiumNotesHtml_HI';
              existingHindi = contentData?.[htmlKeyHI];
              targetKey = htmlKeyHI;
          } else {
              if (syllabusMode === 'SCHOOL') targetKey = 'schoolFreeNotesHtml_HI';
              else targetKey = 'competitionFreeNotesHtml_HI';
              existingHindi = contentData?.[targetKey];
          }

          if (existingHindi) {
              setActivePdf(existingHindi);
              setActiveLang('HINDI');
              return;
          }

          // 4. GENERATE HINDI (AI)
          if (confirm("Hindi translation not found. Generate it using AI? (Takes ~30s)")) {
              setIsTranslating(true);
              try {
                  const translated = await translateToHindi(activePdf);
                  ...
                  localStorage.setItem(contentKey, JSON.stringify(updatedData));
                  
                  // Save Cloud (Best Effort)
                  await saveChapterData(contentKey, updatedData);
                  
                  setActivePdf(translated);
                  setActiveLang('HINDI');
                  setAlertConfig({isOpen: true, message: "✅ Translation Saved Forever!"});
              } catch (e) {
                  console.error(e);
                  setAlertConfig({isOpen: true, message: "Translation Failed. Try again."});
              } finally {
       ...
       <div className="sticky top-0 z-20 bg-white border-b border-slate-100 shadow-sm p-4 flex items-center gap-3">
           ...
           {activePdf && !activePdf.startsWith('http') && (
               <div className="flex bg-slate-100 p-1 rounded-lg mr-2">
                   <button 
                       onClick={() => handleSmartLanguageToggle('ENGLISH')}
                   ...
                   <button 
                       onClick={() => handleSmartLanguageToggle('HINDI')}
                       disabled={isTranslating}
                       className={`px-2 py-1 text-[10px] font-bold rounded transition-all flex items-center gap-1 ${activeLang === 'HINDI' ? 'bg-white shadow text-orange-600' : 'text-slate-500'}`}
                   >
                       {isTranslating ? <Sparkles size={10} className="animate-spin text-orange-500" /> : 'हिन्दी'}
                   </button>
               </div>
           )}
               ...
               <div style={{
                   ...
                   {activePdf.startsWith('http') ? (
                       ...
                       >
                           ...
                           {(() => {
                               // Handle Bilingual Content Split (Legacy |||)
                               // For new separate fields, activePdf IS the correct language content already
